...
type GoodsCommonAddUpdate struct {
	...
	Level1CategoryId int    `json:"level1_categoryId"  description:"1级分类id"`
	Level2CategoryId int    `json:"level2_categoryId"  description:"2级分类id"`
	Level3CategoryId int    `json:"level3_categoryId"  description:"3级分类id"`
	Brand            string `json:"brand"              description:"品牌" v:"max-length:30#品牌名字符不能超过30"`
...
type GoodsUpdateReq struct {
	g.Meta `path:"/goods/update/{Id}" method:"post" tags:"商品" summary:"修改商品接口"`
	Id     uint `json:"id"         v:"min:1#请选择需要修改的内容" dc:"商品Id"`
	GoodsCommonAddUpdate
}
